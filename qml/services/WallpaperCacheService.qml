...
QtObject {
    ...
    function processFiles(files) {
        ...
        for (var i = 0; i < files.length; i++) {
            var f = files[i]
            var thumbName = f.name.replace(/\//g, "--") + ".webp"
        ...
        Process {
            ...
            onExited: {
                ...
                var item = {
                    ...
                    mtime: "0", thumbPath: service.weCacheDir + "/" + weProbe._weId + ".webp",
                    title: title
                }
    ...
    function _buildScanScript() {
        ...
        return 'set -e\n' +
            ...
            'vid_cache=' + vidCache + '\n' +
            'mkdir -p "$vid_dir"\n' +
            'find "$wall_dir" -type f ' + ImageService.findExtPattern(ImageService.imageExtensions) + ' -print0 2>/dev/null | sort -z | while IFS= read -r -d "" img; do\n' +
            '  name="${img#$wall_dir/}"\n' +
            '  mtime=$(stat -c "%Y" "$img" 2>/dev/null || echo 0)\n' +
            '  thumb_name="$(echo "$name" | sed "s|/|--|g").webp"\n' +
            ...
            '    echo "we\t$preview\t$id\t$mtime\t$we_cache/$id.webp\t$title"\n' +
            '  done\n' +
            'fi\n'
    }
